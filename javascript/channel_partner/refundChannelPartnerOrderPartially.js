...
export class RefundChannelPartnerOrderPartially {
  static async execute() {
    ...
    const orderId = orderIdMaybe;
    const getResponse = await new Promise((resolve, reject) => {
        channelPartnerApi.getChannelPartnerOrder(orderId,{_expand: expansion}, function (error, data, response) {
            if (error) {
                reject(error);
            } else {
                resolve(data, response);
    ...
    const refundResponse = await new Promise((resolve, reject) => {
        channelPartnerApi.refundChannelPartnerOrder(orderId, order, {
  ...
  static generateSecureRandomString(length = 10) {
