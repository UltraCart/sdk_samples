...
export class GetDistributionCenterOrders {
    ...
    */
    public static async Execute(): Promise<void> {
        try {
            const acknowledgedOrders: string[] = [];
            const distributionCenterCode: string = "RAMI";

            // Get distribution center orders
            const result: OrdersResponse = await fulfillmentApi.getDistributionCenterOrders({distributionCenterCode});
            ...
            if (acknowledgedOrders.length > 0) {
                await fulfillmentApi.acknowledgeOrders({
                    distributionCenterCode: distributionCenterCode,
                    orderIds: acknowledgedOrders
                });
            }
